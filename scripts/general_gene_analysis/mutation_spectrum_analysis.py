...
OUTPUT_DIR = f"{BASE_DIR}/mutation_spectrum_results"
GENE_OUTPUT_DIR = f"{BASE_DIR}/gene_mutation_spectrum_results"
...
GENE_MAPPING_FULL_FILE = "reference/gene_mapping_full.tsv"
GENES_OF_INTEREST_FILE = "reference/genes_of_interest_mapping.tsv"
CHROMOSOME_MAPPING_FILE = "reference/chromosome_mapping.tsv"
...
def load_gene_mapping():
    ...
    1. The comprehensive gene mapping file (gene_mapping_full.tsv) as primary source
    2. The legacy gene mapping file (gene_mapping.tsv) as fallback
    3. The genes of interest file with ergosterol pathway genes
    4. The chromosome mapping file for ID conversion
    ...
    # Initialize chromosome ID mapping
    chrom_id_to_scaffold = {}
    if os.path.exists(CHROMOSOME_MAPPING_FILE):
        with open(CHROMOSOME_MAPPING_FILE, 'r') as f:
            reader = csv.DictReader(f, delimiter='\t')
            for row in reader:
                chrom_id_to_scaffold[row['chromosome_id']] = row['w303_scaffold']
        print(f"Loaded {len(chrom_id_to_scaffold)} chromosome ID mappings")
    else:
        print(f"Warning: Chromosome mapping file {CHROMOSOME_MAPPING_FILE} not found")
    
    # Dictionary to track which file provided each gene (for debugging)
    gene_sources = {}
    
    # First try loading the comprehensive gene mapping file
    if os.path.exists(GENE_MAPPING_FULL_FILE):
        print(f"Loading comprehensive gene mapping from {GENE_MAPPING_FULL_FILE}")
        
        with open(GENE_MAPPING_FULL_FILE, 'r') as f:
            reader = csv.DictReader(f, delimiter='\t')
            for row in reader:
                gene_id = row['w303_gene_id']
                
                # Store data with chromosome_id included
                GENE_DATA[gene_id] = {
                    'sc_gene_id': row['sc_gene_id'],
                    'erg_name': row.get('erg_name', ''),
                    'locus_tag': row['locus_tag'],
                    'w303_scaffold': row['w303_scaffold'],
                    'chromosome_id': row.get('chromosome_id', ''),
                    'start': int(row['start']),
                    'end': int(row['end']),
                    'strand': row['strand'],
                    'product': row.get('product', 'hypothetical protein'),
                    'is_erg': row.get('is_erg', '').lower() == 'yes'
                }
                
                # Add to scaffold lookup
                SCAFFOLD_GENES[row['w303_scaffold']].append(gene_id)
                
                # Also add to chromosome_id lookup if available
                if row.get('chromosome_id') and row['chromosome_id'] != '':
                    SCAFFOLD_GENES[row['chromosome_id']].append(gene_id)
                
                gene_sources[gene_id] = 'gene_mapping_full.tsv'
        
        print(f"Loaded information for {len(GENE_DATA)} genes from comprehensive mapping")
    
    # Load the legacy gene mapping file as fallback or supplement
    if os.path.exists(GENE_MAPPING_FILE):
        print(f"Loading legacy gene mapping from {GENE_MAPPING_FILE}")
        ...
        with open(GENE_MAPPING_FILE, 'r') as f:
            ...
            for row in reader:
                ...
                # Only add if not already present from comprehensive file
                if gene_id not in GENE_DATA:
                    GENE_DATA[gene_id] = {
                        ...
                        'locus_tag': row['locus_tag'],
                        'w303_scaffold': row['w303_scaffold'],
                        'chromosome_id': '',  # Not available in legacy file
                        'start': int(row['start']),
                        'end': int(row['end']),
                        'strand': row['strand'],
                        'product': row.get('product', 'hypothetical protein'),
                        'is_erg': False  # Default for legacy file
                    }
                    
                    # Add to scaffold lookup
                    SCAFFOLD_GENES[row['w303_scaffold']].append(gene_id)
                    
                    # Try to map to chromosome_id if available
                    if row['w303_scaffold'] in chrom_id_to_scaffold.values():
                        # Find chromosome_id for this scaffold
                        for chrom_id, scaffold in chrom_id_to_scaffold.items():
                            if scaffold == row['w303_scaffold']:
                                SCAFFOLD_GENES[chrom_id].append(gene_id)
                                GENE_DATA[gene_id]['chromosome_id'] = chrom_id
                                break
                    
                    gene_sources[gene_id] = 'gene_mapping.tsv'
                    legacy_count += 1
        
        print(f"Added {legacy_count} genes from legacy mapping")
    ...
    if os.path.exists(GENES_OF_INTEREST_FILE):
        with open(GENES_OF_INTEREST_FILE, 'r') as f:
            ...
            for row in reader:
                ...
                # Update the gene data to mark it as an ergosterol gene
                if gene_id in GENE_DATA:
                    GENE_DATA[gene_id]['is_erg'] = True
                    GENE_DATA[gene_id]['erg_name'] = row.get('erg_name', '')
                    erg_genes_added += 1
        
        print(f"Loaded {len(GENES_OF_INTEREST)} genes of interest")
        print(f"Updated {erg_genes_added} genes with ergosterol pathway information")
    else:
    ...
    scaffolds_count = len([k for k in SCAFFOLD_GENES.keys() if k.startswith('scaffold')])
    chrom_count = len([k for k in SCAFFOLD_GENES.keys() if not k.startswith('scaffold')])
    
    print(f"Final gene mapping contains {total_genes} genes across {scaffolds_count} scaffolds and {chrom_count} chromosomes")
    print(f"Scaffold-based gene counts: {sum(len(genes) for scaffold, genes in SCAFFOLD_GENES.items() if scaffold.startswith('scaffold'))}")
    print(f"Chromosome-based gene counts: {sum(len(genes) for chrom, genes in SCAFFOLD_GENES.items() if not chrom.startswith('scaffold'))}")
...
def map_variants_to_genes(data, upstream_distance=1000):
    ...
    and positional context (coding, upstream, etc.). It handles both scaffold
    and chromosome IDs for more robust mapping.
    ...
    annotated_data['Is_Erg_Pathway'] = False
    
    # Initialize counters for diagnostic output
    total_variants = len(annotated_data)
    scaffolds_found = set()
    scaffolds_missing = set()
    scaffolds_mapped = {}  # Track mapping success by scaffold
    chromosomes_mapped = {}  # Track mapping success by chromosome ID
    ...
    for i, variant in annotated_data.iterrows():
        ...
        # Record if this is a scaffold or chromosome ID
        is_scaffold = chrom.lower().startswith('scaffold')
        if is_scaffold:
            scaffolds_found.add(chrom)
        
        # Check if this chromosome/scaffold has genes
        if chrom in SCAFFOLD_GENES:
            # Direct match in our mapping
            for gene_id in SCAFFOLD_GENES[chrom]:
                gene_info = GENE_DATA[gene_id]
                
                # Only consider genes on this exact scaffold/chromosome
                if is_scaffold and gene_info['w303_scaffold'] != chrom:
                    continue
                elif not is_scaffold and gene_info.get('chromosome_id', '') != chrom:
                    continue
                
                # Check if the variant is within or near the gene
                if _check_gene_overlap(pos, gene_info, upstream_distance):
                    context, match_found = _determine_gene_context(pos, gene_info, upstream_distance)
                    
                    # If the variant affects this gene, add annotation
                    if match_found:
                        _annotate_variant(annotated_data, i, gene_id, gene_info, context)
                        
                        # Update mapping statistics
                        if is_scaffold:
                            scaffolds_mapped[chrom] = scaffolds_mapped.get(chrom, 0) + 1
                        else:
                            chromosomes_mapped[chrom] = chromosomes_mapped.get(chrom, 0) + 1
                        
                        break  # Assign to first matching gene
        else:
            # Try to find a matching scaffold/chromosome through conversion
            if is_scaffold:
                # This is a scaffold ID, try to find matching chromosome genes
                scaffolds_missing.add(chrom)
                
                # Attempt to find corresponding chromosome ID
                for gene_id, gene_info in GENE_DATA.items():
                    if gene_info['w303_scaffold'] == chrom:
                        # Found a gene with this scaffold - check for overlap
                        if _check_gene_overlap(pos, gene_info, upstream_distance):
                            context, match_found = _determine_gene_context(pos, gene_info, upstream_distance)
                ...
                # This is a chromosome ID, try scaffold-based lookup
                for gene_id, gene_info in GENE_DATA.items():
                    if gene_info.get('chromosome_id', '') == chrom:
                        # Found a gene with this chromosome ID - check for overlap
                        if _check_gene_overlap(pos, gene_info, upstream_distance):
                            context, match_found = _determine_gene_context(pos, gene_info, upstream_distance)
    ...
    mapped_percentage = (mapped_variants / total_variants * 100) if total_variants > 0 else 0
    
    # Generate detailed mapping report
    print(f"Mapped {mapped_variants} out of {total_variants} variants to genes ({mapped_percentage:.1f}%)")
    print(f"Found {len(scaffolds_found)} unique scaffolds in the data")
    print(f"Successfully mapped variants on {len(scaffolds_mapped)} scaffolds")
    
    # Report most common missing scaffolds (if any)
    if scaffolds_missing:
        print(f"Warning: {len(scaffolds_missing)} scaffolds had no direct mapping")
        most_common_missing = sorted(scaffolds_missing, key=lambda x: x)[:5]
        print(f"Example missing scaffolds: {', '.join(most_common_missing)}")
    
    # Report chromosome ID mapping statistics
    if chromosomes_mapped:
        print(f"Successfully mapped variants on {len(chromosomes_mapped)} chromosome IDs")
        print(f"Chromosome mapping counts: {chromosomes_mapped}")
    
    return annotated_data

def _check_gene_overlap(pos, gene_info, upstream_distance):
    """
    Check if a position overlaps with a gene or its upstream region.
    
    Args:
        pos (int): The position to check
        gene_info (dict): Dictionary with gene information
        upstream_distance (int): Distance upstream to consider
        
    Returns:
        bool: True if the position overlaps with the gene or its upstream region
    """
    gene_start = gene_info['start']
    gene_end = gene_info['end']
    
    # Check if inside gene
    if gene_start <= pos <= gene_end:
        return True
    
    # Check if in upstream region for + strand gene
    if gene_info['strand'] == '+' and gene_start - upstream_distance <= pos < gene_start:
        return True
    
    # Check if in upstream region for - strand gene
    if gene_info['strand'] == '-' and gene_end < pos <= gene_end + upstream_distance:
        return True
    
    return False

def _determine_gene_context(pos, gene_info, upstream_distance):
    """
    Determine the context of a position relative to a gene.
    ...
    Returns:
        tuple: (context, match_found) pair
    """
    gene_start = gene_info['start']
    gene_end = gene_info['end']
...
def _annotate_variant(data, idx, gene_id, gene_info, context):
    """
    Annotate a variant with gene information.
    
    Args:
        data (pd.DataFrame): DataFrame to update
        idx (int): Index of the variant to annotate
        gene_id (str): Gene ID
        gene_info (dict): Dictionary with gene information
        context (str): Gene context (coding, upstream, etc.)
    """
    # Use erg_name if available, otherwise use sc_gene_id
    gene_name = gene_info.get('erg_name', '') or gene_info.get('sc_gene_id', '')
    
    data.at[idx, 'Gene_ID'] = gene_id
    data.at[idx, 'Gene_Name'] = gene_name
    data.at[idx, 'Gene_Function'] = gene_info.get('product', 'hypothetical protein')
    data.at[idx, 'Gene_Context'] = context
    data.at[idx, 'Is_Gene_Of_Interest'] = gene_id in GENES_OF_INTEREST
    data.at[idx, 'Is_Erg_Pathway'] = gene_info.get('is_erg', False)
