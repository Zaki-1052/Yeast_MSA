...
# File: scripts/annotation/12_extract_target_genes.py
...
def main():
    ...
    if not combined_df.empty:
        ...
        print(f"Saved {len(combined_df)} variants to all_target_variants.tsv")
    else:
    ...
    if not combined_df.empty:
        ...
        grouped = combined_df.groupby(['Gene', 'Chromosome', 'Position', 'Ref', 'Alt'])
        common_variants = []
        ...
        if common_variants:
            ...
            common_df = common_df.sort_values(['Sample_Count', 'Gene'], ascending=[False, True])
            common_df.to_csv(f"{gene_dest}/common_variants.tsv", sep='\t', index=False)
    ...
    print(f"- {gene_dest}/common_variants.tsv")
    print("")
