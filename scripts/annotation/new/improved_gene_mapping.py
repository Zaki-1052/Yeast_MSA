# File: scripts/annotation/new/improved_gene_mapping.py
...
def extract_gene_mapping():
    ...
    for gb_file in gbk_files:
        ...
        try:
            ...
            # Process both gene and CDS features for standard IDs
            for feature in record.features:
                if feature.type in ["gene", "CDS"]:
                    # We'll count each gene feature for stats
                    ...
                    # Check for standard ID in inference field
                    if 'inference' in feature.qualifiers:
                        for inf in feature.qualifiers['inference']:
                            # Print a few examples to debug
                            if ygap_id.startswith("W3030") and total_genes < 10:
                                print(f"Inference example: {inf}")
                            match = re.search(r':([A-Z0-9]{2,3}\d+[WC])', inf)
                            if match:
                                standard_id = match.group(1)
                    
                    # Check for standard ID in note field
                    if 'note' in feature.qualifiers:
                        for note in feature.qualifiers['note']:
                            # Print a few examples to debug
                            if ygap_id.startswith("W3030") and total_genes < 10:
                                print(f"Note example: {note}")
                            match = re.search(r'([A-Z0-9]{2,3}\d+[WC])', note)
                            if match:
                                standard_id = match.group(1)
                    
                    # Store the mapping if we found a standard ID
                    if standard_id:
